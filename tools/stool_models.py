...
def get_item_model(color):
    return {
        "parent": f"cobblefurnies:block/stool/{color}"
    }

def get_block_model(color):
    return {
        "parent": "cobblefurnies:block/template/stool",
        "textures": {
            "0": f"cobblefurnies:block/stool/{color}"
        }
    }
...
def get_blockstate(color):
    return {
        "variants": {
            "facing=north": {
                "model": f"cobblefurnies:block/stool/{color}"
            },
            "facing=east": {
                "model": f"cobblefurnies:block/stool/{color}",
                "y": 90
...
def get_lang_entry(color):
    return {f"block.cobblefurnies.{color}_stool": ""}
...
def generate_files():
    ...
    for color in colors:
        # Item model
        item_model_path = os.path.join(base_dir, f"models/item/{color}_stool.json")
        save_json_file(item_model_path, get_item_model(color))

        # Block model
        block_model_path = os.path.join(base_dir, f"models/block/stool/{color}.json")
        save_json_file(block_model_path, get_block_model(color))

        # Blockstate
        blockstate_path = os.path.join(base_dir, f"blockstates/{color}_stool.json")
