...
public class FridgeBlockEntity extends RandomizableContainerBlockEntity {
    ...
    public FridgeBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(CFBlockEntityTypes.FRIDGE.get(), blockPos, blockState);
        this.items = NonNullList.withSize(27, ItemStack.EMPTY);
        this.openersCounter = new ContainerOpenersCounter() {
            protected void onOpen(Level level, BlockPos pos, BlockState state) {
                FridgeBlockEntity.this.playSound(CFSoundEvents.FRIDGE_OPEN.get());
                FridgeBlockEntity.this.updateBlockState(state, true);
            }

            protected void onClose(Level level, BlockPos pos, BlockState state) {
                FridgeBlockEntity.this.playSound(CFSoundEvents.FRIDGE_CLOSE.get());
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.cobblefurnies.fridge");
    }
