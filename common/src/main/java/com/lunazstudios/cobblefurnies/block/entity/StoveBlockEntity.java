...
public class StoveBlockEntity extends RandomizableContainerBlockEntity {
    private NonNullList<ItemStack> items;
    private ContainerOpenersCounter openersCounter;

    public StoveBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(CFBlockEntityTypes.STOVE.get(), blockPos, blockState);
        this.items = NonNullList.withSize(27, ItemStack.EMPTY);
        this.openersCounter = new ContainerOpenersCounter() {
            protected void onOpen(Level level, BlockPos pos, BlockState state) {
                StoveBlockEntity.this.playSound(state, SoundEvents.BARREL_OPEN);
                StoveBlockEntity.this.updateBlockState(state, true);
            }

            protected void onClose(Level level, BlockPos pos, BlockState state) {
                StoveBlockEntity.this.playSound(state, SoundEvents.BARREL_CLOSE);
                StoveBlockEntity.this.updateBlockState(state, false);
            }
            ...
            protected boolean isOwnContainer(Player player) {
                if (player.containerMenu instanceof ChestMenu) {
                    ...
                    return container == StoveBlockEntity.this;
                } else {
                    return false;
                }
            }
    ...
    void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), state.setValue(StoveBlock.OPEN, open), 3);
    }
